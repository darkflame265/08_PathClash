...
export function resolveAbilityRound(params: {
  ...
  redState: Pick<AbilityPlayerState, 'position' | 'hp' | 'mana' | 'invulnerableSteps'>;
  blueState: Pick<AbilityPlayerState, 'position' | 'hp' | 'mana' | 'invulnerableSteps'>;
  ...
  for (let step = 0; step <= maxStep; step++) {
    if (step > 0) {
      ...
      if (step <= redPath.length) redPos = { ...redPath[step - 1] };
      if (step <= bluePath.length) bluePos = { ...bluePath[step - 1] };
