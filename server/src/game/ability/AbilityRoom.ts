...
function buildNoBacktrackTargetHitPath(
  start: Position,
  targetPos: Position,
  pathPoints: number,
  obstacles: Position[],
): Position[] {
  if (pathPoints <= 0) return [];

  let bestPath: Position[] = [];
  let bestScore = -Infinity;
  const startsOnTarget = posEqual(start, targetPos);

  const scorePath = (path: Position[]): number => {
    ...
    if (targetSteps.length < requiredTargetVisits) return -Infinity;
    if (damagingTargetSteps.length === 0) return -Infinity;

    const lateHitScore = damagingTargetSteps.reduce(
      (sum, step) => sum + Math.max(0, pathPoints - step + 1),
      0,
    );
    const finalPosition = path[path.length - 1] ?? start;
    return (
      damagingTargetSteps.length * 10000 +
  ...
  const visit = (
    ...
    if (path.length >= pathPoints) return;
...
export class AbilityRoom {
  ...
  private trainingSkillSelectionPending = false;
  ...
  private revealPlans(): void {
    ...
    red.previousTurnPath = buildPreviousTurnPathWithQuantumShift(
      'red',
      resolution.payload.redPath,
      resolution.payload.skillEvents,
    );
    blue.previousTurnStart = { ...resolution.payload.blueStart };
    blue.previousTurnPath = buildPreviousTurnPathWithQuantumShift(
      'blue',
  ...
  private validatePlan(player: AbilityPlayerState, path: Position[], skills: AbilitySkillReservation[]): { path: Position[]; skills: AbilitySkillReservation[] } | null {
    ...
    const movementObstacles = [
      ...
      (position) => !this.iceFieldTiles.some((tile) => posEqual(tile.position, position)),
    );
  ...
  private buildAnnihilationCandidates(
    ...
    const hitPath: Position[] = isGuardTarget
    ...
    for (const skillId of bot.equippedSkills) {
      ...
      if (skillId === 'ember_blast') {
        ...
        const firstTargetStep =
          targetSteps.find((step) => step >= minAttackSkillStep) ?? -1;
