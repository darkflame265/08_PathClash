...
exports.getLastAiEscapeDebug = getLastAiEscapeDebug;
...
let lastAiEscapeDebug = null;
function getLastAiAttackDebug() {
    return lastAiAttackDebug;
}
function getLastAiEscapeDebug() {
...
function extendEscapePathToFullPoints(start, basePath, threatPosition, pathPoints, obstacles, threatCandidates) {
    const path = [...basePath].slice(0, pathPoints);
    if (path.length >= pathPoints)
        return path;
    let current = path[path.length - 1] ?? start;
    let previous = path.length >= 1 ? (path.length >= 2 ? path[path.length - 2] : start) : null;
    const visited = new Set([toKey(start), ...path.map((p) => toKey(p))]);
    while (path.length < pathPoints) {
        const candidates = getNeighbors(current, obstacles)
        ...
        const nextMove = candidates
            .map((c) => ({
            candidate: c,
            score: manhattan(c, threatPosition) * 4.5 -
...
function createEscaperPath(selfPosition, threatPosition, pathPoints, obstacles) {
    const threatCandidates = buildThreatAttackCandidates(threatPosition, selfPosition, pathPoints, obstacles);
    const escapeCandidates = buildEscapePathCandidates(selfPosition, threatPosition, pathPoints, obstacles);
    const futureThreatCache = new Map();
    ...
    const chosenPath = extendEscapePathToFullPoints(selfPosition, rawChosenPath, threatPosition, pathPoints, obstacles, threatCandidates);
    lastAiEscapeDebug = {
...
function buildThreatAttackCandidates(attackerPosition, targetPosition, pathPoints, obstacles) {
    ...
    for (const anchor of attackTargets) {
        const attackCandidates = buildAttackPathCandidates(attackerPosition, anchor.target, pathPoints, obstacles);
        for (const path of attackCandidates) {
            ...
            const minDistanceToTarget = Math.min(...sequence.map((step) => manhattan(step, targetPosition)));
            const minDistanceToAnchor = Math.min(...sequence.map((step) => manhattan(step, anchor.target)));
            const finalDistance = manhattan(finalPosition, targetPosition);
            const pressure = Math.max(0, 5 - minDistanceToTarget) * 2.8 +
...
function buildEscapePathCandidates(start, threat, pathPoints, obstacles) {
    ...
    for (const boardTarget of listBoardPositions().filter((candidate) => !isBlocked(candidate, obstacles) && !sameCell(candidate, start))) {
        const shortestPath = buildShortestPath(start, boardTarget, obstacles).slice(0, maxDepth);
...
function scoreEscapePathAgainstEnemyAttackCandidates(start, threatPosition, candidatePath, threatCandidates, obstacles, pathPoints, futureThreatCache) {
    ...
    for (const threatCandidate of threatCandidates.candidates) {
        ...
        for (let step = 0; step < maxSteps; step++) {
            const escapeCurrent = escapeSequence[Math.min(step, escapeSequence.length - 1)];
            const escapePrev = step > 0
                ? escapeSequence[Math.min(step - 1, escapeSequence.length - 1)]
                : escapeSequence[0];
            const attackCurrent = attackSequence[Math.min(step, attackSequence.length - 1)];
            const attackPrev = step > 0
            ...
            if (sameCell(escapeCurrent, attackCurrent)) {
            ...
            else if (sameCell(escapePrev, attackCurrent) &&
                ...
                else if (sameCell(escapeCurrent, attackPrev) ||
    ...
    const timedDangerExposure = escapeSequence.reduce((sum, position, step) => {
        return (sum + (threatCandidates.timeHeatmap.get(`${step}:${toKey(position)}`) ?? 0));
    }, 0);
    const bottleneckDanger = sumHeat(uniqueEscapeKeys, threatCandidates.bottleneckHeatmap);
    const finalPosition = candidatePath[candidatePath.length - 1] ?? start;
    const finalMobility = countOpenNeighbors(finalPosition, obstacles);
    const futureMobility = getNeighbors(finalPosition, obstacles).reduce((sum, neighbor) => sum + countOpenNeighbors(neighbor, obstacles), 0);
    ...
    const directLinePenalty = isDirectRetreatLine(start, finalPosition, threatPosition)
        ? 1
        : 0;
    const futureThreat = getFutureThreatModel(threatPosition, finalPosition, pathPoints, obstacles, futureThreatCache);
    ...
    const futureBottleneckDanger = sumHeat(finalRegionKeys, futureThreat.bottleneckHeatmap);
    const safeExitCount = getNeighbors(finalPosition, obstacles).filter((neighbor) => (futureThreat.heatmap.get(toKey(neighbor)) ?? 0) <=
        (futureThreat.heatmap.get(toKey(finalPosition)) ?? 0)).length;
...
function scoreEscapeExpansionHeuristic(candidate, threatPosition, depth, obstacles) {
    return (countOpenNeighbors(candidate, obstacles) * 6.1 +
        getNeighbors(candidate, obstacles).reduce((sum, neighbor) => sum + countOpenNeighbors(neighbor, obstacles), 0) * 0.9 +
...
function buildThreatAttackTargets(attackerPosition, targetPosition, obstacles) {
    const byKey = new Map();
    const register = (target, priority) => {
    ...
    for (const neighbor of getNeighbors(targetPosition, obstacles)) {
        register(neighbor, 22 + Math.max(0, 4 - manhattan(neighbor, attackerPosition)) * 2);
    }
    for (const cell of listBoardPositions()) {
        if (isBlocked(cell, obstacles) || sameCell(cell, targetPosition))
            continue;
        const distanceToTarget = manhattan(cell, targetPosition);
        if (distanceToTarget > 3)
            continue;
        const mobility = countOpenNeighbors(cell, obstacles);
        const priority = Math.max(0, 5 - distanceToTarget) * 3 +
            (isEdge(cell) ? 0.5 : 2.4) +
            mobility * 0.8;
...
function getFutureThreatModel(threatPosition, targetPosition, pathPoints, obstacles, cache) {
...
function pickWeightedTopThree(candidates) {
    ...
    if (roll < 0.5 || candidates.length <= 1)
        return candidates[0];
    if (roll < 0.8 || candidates.length <= 3)
        return candidates[Math.min(3, candidates.length - 1)];
    return candidates[Math.min(6, candidates.length - 1)];
}
...
function isDirectRetreatLine(start, end, threat) {
