...
class AbilityRoom {
    ...
    revealPlans() {
        ...
        const atomicCloneMaxSteps = resolution.payload.skillEvents.reduce((max, event) => {
            if (event.skillId !== 'atomic_fission' || !event.cloneStart)
                return max;
            return Math.max(max, event.step + (event.clonePath?.length ?? 0));
        }, 0);
        const animTime = (0, GameEngine_1.calcAnimationDuration)(Math.max(resolution.payload.redPath.length, resolution.payload.bluePath.length, atomicCloneMaxSteps) + resolution.payload.skillEvents.length) +
    ...
    buildBotSkillActionCandidates(bot, opponent, pathPoints, selfModel, opponentModel, effectiveObstacles) {
        ...
        for (const skillId of bot.equippedSkills) {
            ...
            if (skillId === 'phase_shift') {
                ...
                const currentDanger = scoreEscapePathAgainstModel(bot.position, [], opponent.position, opponentModel, effectiveObstacles);
                const bestBaseEscapeScore = basePaths.reduce((best, path) => Math.max(best, scoreEscapePathAgainstModel(bot.position, path, opponent.position, opponentModel, effectiveObstacles)), -Infinity);
                const closeRangeBonus = phaseDistanceToOpponent <= 1
                    ? 760
                    : phaseDistanceToOpponent <= 2
                        ...
                        : phaseDistanceToOpponent <= 3
                            ? 260
                            : 0;
                const hpPressureBonus = bot.hp <= 2 ? 560 : bot.hp === 3 ? 360 : bot.hp === 4 ? 160 : 0;
                const threatBonus = isUnderSkillThreat ? 1100 : 0;
                const dangerBonus = currentDanger < -360 ? 680 : currentDanger < -180 ? 420 : 140;
                const manaOverflowBonus = bot.mana >= MAX_MANA ? 280 : bot.mana >= 9 ? 150 : 0;
                const lowUrgencyPenalty = !isUnderSkillThreat && phaseDistanceToOpponent > 3 && bot.hp >= 4 && bot.mana < MAX_MANA
                    ? 520
                    : 0;
                const phaseBonus = 480 +
                    threatBonus +
                    closeRangeBonus +
                    hpPressureBonus +
                    dangerBonus +
                ...
                for (const path of basePaths.slice(0, 4)) {
                    const base = this.scoreBotActionCandidate(bot, opponent, path, [{ skillId, step: 0, order: 0 }], opponentModel, 'phase_shift-escape-invulnerable', effectiveObstacles);
                    const escapeImprovement = Number.isFinite(bestBaseEscapeScore) ? Math.max(0, base.score - bestBaseEscapeScore) : 0;
                    candidates.push({
                        ...base,
            ...
            if (skillId === 'void_cloak') {
                ...
                const currentDanger = scoreEscapePathAgainstModel(bot.position, [], opponent.position, opponentModel, effectiveObstacles);
                const closeRangeBonus = cloakDistanceToOpponent <= 1
                    ? 520
                    : cloakDistanceToOpponent <= 2
                        ...
                        : cloakDistanceToOpponent <= 3
                            ? 180
                            : 0;
                const hpPressureBonus = bot.hp <= 2 ? 420 : bot.hp === 3 ? 260 : bot.hp === 4 ? 120 : 0;
                const threatBonus = isUnderSkillThreat ? 900 : 0;
                const dangerBonus = currentDanger < -300 ? 520 : currentDanger < -150 ? 320 : 140;
                const manaOverflowBonus = bot.mana >= MAX_MANA ? 260 : bot.mana >= 8 ? 130 : 0;
                const repeatPenalty = usedFourCostLastTurn && !isUnderSkillThreat && cloakDistanceToOpponent > 2
                    ? 280
                    : 0;
                const cloakBonus = 420 +
                    threatBonus +
                    closeRangeBonus +
                    hpPressureBonus +
                    dangerBonus +
                ...
                for (const path of basePaths.slice(0, 3)) {
                    const base = this.scoreBotActionCandidate(bot, opponent, path, [{ skillId, step: 0, order: 0 }], opponentModel, 'void_cloak-escape-setup', effectiveObstacles);
                    candidates.push({
                        ...base,
