...
function isMovingPersistentOverlap(aNow, aPrev, bNow, bPrev) {
    return (isPersistentOverlap(aNow, aPrev, bNow, bPrev) &&
...
function updateIceFieldTile(iceFieldTiles, position, remainingTurns) {
...
function computeSlidePath(from, direction, blockedPositions) {
    const slide = [];
    let row = from.row;
    let col = from.col;
    for (;;) {
        ...
        if (nr < 0 || nr > 4 || nc < 0 || nc > 4)
            break;
        if (blockedPositions.some((position) => position.row === nr && position.col === nc))
            break;
        row = nr;
        col = nc;
        slide.push({ row, col });
...
function withoutIceFieldPositions(positions, activeIceFieldTiles) {
    return positions.filter((position) => !activeIceFieldTiles.some((tile) => samePosition(tile.position, position)));
}
...
function resolveAbilityRound(params) {
    ...
    const activeIceFieldTiles = params.iceFieldTiles.map((tile) => ({
        position: { ...tile.position },
        remainingTurns: tile.remainingTurns,
    }));
    ...
    let redIceSlideOverriddenPath = null;
    let blueIceSlideOverriddenPath = null;
    ...
    for (let step = 0; step <= maxStep; step++) {
        ...
        if (step > 0) {
            ...
            const redHeldByRootWall = redStoppedByRootWall && step <= redPath.length && samePosition(redNext, redPrev);
            const blueHeldByRootWall = blueStoppedByRootWall && step <= bluePath.length && samePosition(blueNext, bluePrev);
            const rootWallHeldOverlap = startsStepOverlapped &&
