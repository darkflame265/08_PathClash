...
- 루트장벽 타일 → 장애물 직전 칸에서 정지

다음은 슬라이드를 멈추지 **않는다**:

- **용암지대 타일**: 통과하며 계속 슬라이드. 통과 시 기존 용암 피해 로직(1 HP 차감)이 그대로 적용됨
- **다른 빙판 타일**: 무시하고 같은 방향으로 계속 슬라이드

슬라이드 연장이 0칸인 경우(빙판 칸이 이미 경계이거나 바로 다음 칸이 루트장벽): 빙판 칸 자체에 멈춤.
...
- **슬라이드 중 용암 통과**: 슬라이드를 멈추지 않음. 용암 피해(1 HP) 는 기존 `applyLavaDamage` 로직이 그대로 처리
- **슬라이드 중 두 번째 빙판 진입**: 무시하고 동일 방향으로 계속 슬라이드 (체이닝 없음)
...
-슬라이드 중 용암지대 장애물 만나면 용암지대 타일도 통과할 것. 당연히 용암지대를 통과하며 해당 용암지대 장애물에 의해 hp도 1 깎여야 함. -슬라이드 중 두 번째 빙판을 만나도 무시하고, 그냥 그대로 쭉 미끄러질 것.
