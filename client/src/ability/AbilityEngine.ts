...
import { ABILITY_SKILL_COSTS } from "../types/ability.types";
import type {
  AbilityBlockEvent,
  AbilityDamageEvent,
  AbilityHealEvent,
...
export function resolveAbilityRound(params: {
  ...
  for (let step = 0; step <= maxStep; step++) {
    ...
    const overlappingAfterSkillBlitz =
      (redBlitz || blueBlitz) && samePosition(redPos, bluePos);
    if (
      overlappingAfterSkillBlitz &&
      !samePosition(redPrevForStep, bluePrevForStep)
    ) {
      const landingBlitzColor: PlayerColor | null =
        redBlitz && !redBlitzDamagedThisStep
          ? 'red'
      ...
      if (landingBlitzColor) {
        resolveCollisionHit(
          landingBlitzColor,
          landingBlitzColor === 'red' ? 'blue' : 'red',
          redPos,
          step,
        );
      }
    ...
    const getClonePositionForStep = (
      ...
      if (index < clonePath.length) return { ...clonePath[index] };
    ...
    const applySunChariotHit = (
      ...
      if (alreadyHit || order === null) return alreadyHit;
      ...
      if (damages.length === 0) return alreadyHit;
      applyDamages(
        sourceColor,
        damages,
        [],
    ...
    if (redSunChariot) {
      redSunChariotHit = applySunChariotHit(
        'red',
        redPos,
        bluePos,
        redSunChariotHit,
    ...
    if (blueSunChariot) {
      blueSunChariotHit = applySunChariotHit(
        'blue',
        bluePos,
        redPos,
        blueSunChariotHit,
  ...
  const resolveBlockedPathTail = (
    path: Position[],
    blockedAtStep: number | null,
    stopPosition: Position | null,
  ) => {
    if (blockedAtStep === null || !stopPosition) return null;
    return {
      start: { ...stopPosition },
  ...
  return {
    payload: {
      ...
      rootWallBlockedPaths: {
        red: resolveBlockedPathTail(
          redPath,
          redPathBlockedAtStep,
          redRootWallStopPosition,
        ),
        blue: resolveBlockedPathTail(
          bluePath,
          bluePathBlockedAtStep,
          blueRootWallStopPosition,
        ),
