...
export function AbilityScreen({ onLeaveToLobby }: Props) {
  ...
  const [roundInfo, setRoundInfo] = useState<AbilityRoundStartPayload | null>(null);
  const [myPath, setMyPath] = useState<Position[]>([]);
  const [opponentPath, setOpponentPath] = useState<Position[]>([]);
  const [mySubmitted, setMySubmitted] = useState(false);
  const [opponentSubmitted, setOpponentSubmitted] = useState(false);
  const [skillReservations, setSkillReservations] = useState<AbilitySkillReservation[]>([]);
  const [selectedSkillId, setSelectedSkillId] = useState<AbilitySkillId | null>(null);
  const [pendingExplosionStep, setPendingExplosionStep] = useState(false);
  const [pendingTeleport, setPendingTeleport] = useState(false);
  const [redDisplayPos, setRedDisplayPos] = useState<Position>({ row: 2, col: 0 });
  const [blueDisplayPos, setBlueDisplayPos] = useState<Position>({ row: 2, col: 4 });
  const [hitFlags, setHitFlags] = useState<{ red: boolean; blue: boolean }>({ red: false, blue: false });
  const [explodingFlags, setExplodingFlags] = useState<{ red: boolean; blue: boolean }>({ red: false, blue: false });
  const [collisionEffects, setCollisionEffects] = useState<Array<{ id: number; position: Position }>>([]);
  const [activeGuards, setActiveGuards] = useState<{ red: boolean; blue: boolean }>({ red: false, blue: false });
  const [movingPaths, setMovingPaths] = useState<{ red: Position[]; blue: Position[] }>({ red: [], blue: [] });
  const [winner, setWinner] = useState<PlayerColor | 'draw' | null>(null);
  const [gameOverMessage, setGameOverMessage] = useState<string | null>(null);
  const [rematchRequested, setRematchRequested] = useState(false);
  const [abilityBanner, setAbilityBanner] = useState<string | null>(null);
  const [opponentSkillInfo, setOpponentSkillInfo] = useState<AbilitySkillId | null>(null);
  ...
  const queueAnimationTimeout = (callback: () => void, delay: number) => {
    const timeoutId = window.setTimeout(() => {
      animationTimeoutIdsRef.current = animationTimeoutIdsRef.current.filter((value) => value !== timeoutId);
      callback();
    }, delay);
  ...
  const queueSubmitTimeout = (callback: () => void, delay: number) => {
    const timeoutId = window.setTimeout(() => {
      submitTimeoutIdsRef.current = submitTimeoutIdsRef.current.filter((value) => value !== timeoutId);
      callback();
    }, delay);
  ...
  const getPreviewStart = () => {
    const teleport = skillReservations.find((entry) => entry.skillId === 'quantum_shift' && entry.target);
    return teleport?.target ?? state?.players[currentColor].position ?? { row: 2, col: 0 };
  };

  const getAvailableSkills = () => state?.players[currentColor].equippedSkills ?? [];
  const getSkillCost = (skillId: AbilitySkillId) => ABILITY_SKILLS[skillId].manaCost;
  const getReservedMana = () => skillReservations.reduce((sum, reservation) => sum + getSkillCost(reservation.skillId), 0);
  const getRemainingMana = () => Math.max(0, getMyMana() - getReservedMana());

  const syncMyPlan = (path: Position[], reservations: AbilitySkillReservation[]) => {
  ...
  const updateSkillReservations = (nextReservations: AbilitySkillReservation[]) => {
    setSkillReservations(nextReservations);
    syncMyPlan(myPath, nextReservations);
  };

  const removeReservation = (skillId: AbilitySkillId) => {
    const nextReservations = skillReservations.filter((entry) => entry.skillId !== skillId);
    setSelectedSkillId(null);
    setPendingExplosionStep(false);
    setPendingTeleport(false);
  ...
  const toggleGuardSkill = () => {
    const alreadyReserved = skillReservations.some((entry) => entry.skillId === 'classic_guard');
    if (alreadyReserved) {
    ...
    const nextReservations: AbilitySkillReservation[] = [
      ...
      { skillId: 'classic_guard', step: 0, order: reservationOrderRef.current++ },
    ];
    setMyPath([]);
    setSkillReservations(nextReservations);
    setSelectedSkillId(null);
  ...
  const beginTeleportPick = () => {
    const alreadyReserved = skillReservations.some((entry) => entry.skillId === 'quantum_shift');
    if (alreadyReserved) {
  ...
  const handleTeleportTargetSelect = (target: Position) => {
    const nextReservations: AbilitySkillReservation[] = [
      ...
      { skillId: 'quantum_shift', step: 0, order: reservationOrderRef.current++, target },
    ];
    setMyPath([]);
    setSkillReservations(nextReservations);
    setSelectedSkillId(null);
  ...
  const triggerLocalHit = (color: PlayerColor, hpAfter: number, position: Position) => {
    ...
    queueAnimationTimeout(() => {
      setCollisionEffects((prev) => prev.filter((entry) => entry.id !== effectId));
    }, 600);
  ...
  const runSkillEvent = (event: AbilityResolutionPayload['skillEvents'][number], done: () => void) => {
  ...
  const runAnimation = (payload: AbilityResolutionPayload) => {
    ...
    for (const event of payload.skillEvents) {
      ...
      list.push(event);
      skillMap.set(event.step, list.sort((left, right) => left.order - right.order));
    }

    const collisionMap = new Map<number, AbilityResolutionPayload['collisions'][number]>();
    ...
    const runSkillQueue = (
      ...
      runSkillEvent(events[index], () => runSkillQueue(events, index + 1, done));
    };
    ...
    const advance = (step: number) => {
      ...
      queueAnimationTimeout(() => {
        ...
        if (collision) {
          ...
          triggerLocalHit(collision.escapeeColor, collision.newHp, collision.position);
        }
  ...
  useEffect(() => {
    ...
    const onRoomJoined = ({ roomId, color }: { roomId: string; color: PlayerColor }) => {
      setMyColor(color);
      setRoomCode(roomId);
      socket.emit('ability_client_ready');
    };

    const onPlanUpdated = ({ color, path }: { color: PlayerColor; path: Position[]; skills: AbilitySkillReservation[] }) => {
    ...
    const onPlayerSubmitted = ({ color, path, skills }: { color: PlayerColor; path: Position[]; skills: AbilitySkillReservation[] }) => {
    ...
    const onGameOver = ({ winner: nextWinner }: { winner: PlayerColor | 'draw' }) => {
    ...
    const onOpponentDisconnected = () => {
      ...
      setGameOverMessage(lang === 'en' ? 'The opponent disconnected.' : '상대가 연결을 끊었습니다.');
      setState((prev) => (prev ? { ...prev, phase: 'gameover' } : prev));
    };
  ...
  useEffect(() => {
    if (!state || state.phase !== 'planning' || mySubmitted || !roundInfo) return;
    const socket = getSocket();
    const submitCurrentPlan = () => {
      ...
      socket.emit(
        ...
        ({ ok, acceptedPath, acceptedSkills }: { ok: boolean; acceptedPath: Position[]; acceptedSkills: AbilitySkillReservation[] }) => {
          if (!ok) return;
    ...
    const preSubmitDelay = Math.max(0, roundInfo.roundEndsAt - getEstimatedServerNow() - 250);
    const finalSubmitDelay = Math.max(0, roundInfo.roundEndsAt - getEstimatedServerNow());
  ...
  if (!state) {
    return <div className="gs-loading">{lang === 'en' ? 'Loading ability battle...' : '능력 대전 로딩 중...'}</div>;
  }

  const me = state.players[currentColor];
  const opponent = state.players[opponentColor];
  const rewardTokens = winner && winner === currentColor ? Math.min(6, Math.max(0, 120 - accountDailyRewardTokens)) : 0;
  const isPlanning = state.phase === 'planning';
  const previewStart = getPreviewStart();
  const canDrawPath = isPlanning && !mySubmitted && !skillReservations.some((reservation) => reservation.skillId === 'classic_guard');
  ...
  return (
    <div className="game-screen ability-screen" style={{ '--gs-scale': scale } as CSSProperties}>
      ...
      <div className={`gs-player-card gs-opponent gs-color-${opponentColor}`}>
        <div className="gs-role-badge ability-role-badge">
          <span className="gs-role-icon">{opponent.role === 'attacker' ? 'ATK' : 'RUN'}</span>
          <span className="gs-role-label">
            {opponent.role === 'attacker' ? (lang === 'en' ? 'Attack' : '공격') : (lang === 'en' ? 'Escape' : '도망')}
          </span>
          <div className="ability-opponent-skills">
            {opponent.equippedSkills.map((skillId) => (
              <button
                ...
                onClick={() => setOpponentSkillInfo((current) => current === skillId ? null : skillId)}
              >
        ...
        <div className="gs-player-mid">
          ...
          <span className="gs-color-tag">{opponentColor === 'red' ? 'RED' : 'BLUE'}</span>
          {opponentSubmitted && <span className="gs-color-tag">{lang === 'en' ? 'READY' : '준비 완료'}</span>}
        </div>
        <div className="gs-hp-slot">
          <HpDisplay color={opponentColor} hp={opponent.hp} myColor={currentColor} />
        </div>
      </div>

      {opponentSkillInfo && (
        <div className="ability-skill-tooltip ability-skill-tooltip-top">
          <strong>{lang === 'en' ? ABILITY_SKILLS[opponentSkillInfo].name.en : ABILITY_SKILLS[opponentSkillInfo].name.kr}</strong>
          <span>{lang === 'en' ? ABILITY_SKILLS[opponentSkillInfo].description.en : ABILITY_SKILLS[opponentSkillInfo].description.kr}</span>
        </div>
      )}
          ...
          <div className="gs-result-slot">
            <div className="gameover-overlay">
              <div className="gameover-box">
                <div className={`gameover-result ${winner === currentColor ? 'win' : 'lose'}`}>
                  {winner === 'draw'
                    ? (lang === 'en' ? 'Draw' : '무승부')
                    : winner === currentColor
                      ? (lang === 'en' ? 'YOU WIN!' : '승리')
                ...
                {winner === currentColor && rewardTokens > 0 && (
                  <div className="gameover-reward">
                    {lang === 'en' ? `+${rewardTokens} Tokens` : `+${rewardTokens} 토큰 획득`}
                  </div>
                )}
                {rematchRequested && <div className="rematch-notice">{lang === 'en' ? 'Opponent requested rematch.' : '상대가 재도전을 요청했습니다.'}</div>}
                {rematchRequestSent && <div className="rematch-notice">{lang === 'en' ? 'Rematch request sent.' : '재도전 요청을 보냈습니다.'}</div>}
                {!gameOverMessage && (
                  <button className="rematch-btn" onClick={handleRematch}>{lang === 'en' ? 'REMATCH' : '재도전'}</button>
                )}
              </div>
            </div>
          </div>
      ...
      <div className={`gs-player-card gs-self gs-color-${currentColor} gs-role-${me.role === 'attacker' ? 'atk' : 'run'}`}>
        <div className={`gs-role-badge gs-role-badge-self gs-role-badge-${me.role === 'attacker' ? 'atk' : 'run'}`}>
          <span className="gs-role-icon">{me.role === 'attacker' ? 'ATK' : 'RUN'}</span>
          <span className="gs-role-label">{me.role === 'attacker' ? (lang === 'en' ? 'Attack' : '공격') : (lang === 'en' ? 'Escape' : '도망')}</span>
        </div>
        <div className="gs-player-mid">
          <PlayerInfo player={me} isMe={true} />
          <span className="gs-color-tag">{currentColor === 'red' ? 'RED' : 'BLUE'}</span>
          {mySubmitted && <span className="gs-color-tag">{lang === 'en' ? 'READY' : '준비 완료'}</span>}
        </div>
        <div className="gs-hp-slot">
      ...
      <div className="gs-path-bar">
        <div className="gs-path-header">
          <span className="gs-path-label">{lang === 'en' ? 'Path Points' : '경로 포인트'}</span>
        ...
        <div className="gs-path-gauge">
          {Array.from({ length: state.pathPoints }, (_, index) => (
            <div key={index} className={`gs-path-seg${index < myPath.length ? ' filled' : ''}`} />
          ))}
        </div>
      </div>
      ...
      <div className="ability-skill-panel">
        <div className="ability-skill-panel-head">
          ...
          <span>{lang === 'en' ? `Mana ${getMyMana()} / 10` : `마나 ${getMyMana()} / 10`}</span>
        </div>
        <div className="ability-skill-buttons">
          {getAvailableSkills().map((skillId) => {
            const skill = ABILITY_SKILLS[skillId];
            const reserved = skillReservations.some((entry) => entry.skillId === skillId);
            const disabled = !isPlanning || mySubmitted || (getRemainingMana() < skill.manaCost && !reserved);
            return (
              <button
                key={skillId}
                type="button"
              ...
              >
                ...
                <span className="ability-skill-name">{lang === 'en' ? skill.name.en : skill.name.kr}</span>
        ...
        {pendingExplosionStep && (
          <div className="ability-step-picker">
            <span>{lang === 'en' ? 'Select trigger step' : '발동 시점 선택'}</span>
            <div className="ability-step-picker-row">
              {Array.from({ length: myPath.length + 1 }, (_, index) => (
                <button key={index} type="button" className="ability-step-chip" onClick={() => handleExplosionStepSelect(index)}>
                  {index}
                </button>
              ))}
            </div>
          ...
          {skillReservations.map((reservation) => {
            ...
            return (
              ...
              >
                ...
                <span>{lang === 'en' ? `step ${reservation.step}` : `${reservation.step}칸`}</span>
              </button>
            );
          })}
        </div>
        <div className="ability-mana-bar">
          {Array.from({ length: 10 }, (_, index) => (
            <span key={index} className={`ability-mana-pip ${index < getMyMana() ? 'is-filled' : ''}`} />
          ))}
        </div>
      </div>
    </div>
